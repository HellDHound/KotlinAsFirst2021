 ...
 * Вывести значение того же угла в радианах (например, 0.63256). градусы + (минуты + секунды/60)/60
 */
fun angleInRadian(deg: Int, min: Int, sec: Int): Double = (deg + (min.toDouble() + sec.toDouble() / 60) / 60) * PI / 180
...
fun accountInThreeYears(initial: Int, percent: Int): Double {
    ...
    for (i in 1..3) {
        val a: Double = result + result / 100 * percent
        result = a
    }
    return result
}
